...
class UserController {
    ...
    async getUserByUsername(user: User, username: string): Promise<User>  {
        const result: User = await this.dao.getUserByUsername(username)
        if (user.username !== result.username && user.role !== Role.ADMIN) throw new UnauthorizedUserError()
        return result
    }
    ...
    async deleteUser(user: User, username: string): Promise<Boolean> {
        const result: User = await this.dao.getUserByUsername(username)
        if (user.username !== result.username && user.role !== Role.ADMIN) throw new UserNotAdminError()
        if (user.username !== result.username && result.role === Role.ADMIN) throw new UserIsAdminError()
        const isDeleted: Boolean = await this.dao.deleteUSer(username)
        return isDeleted
    }
    ...
    async updateUserInfo(user: User, name: string, surname: string, address: string, birthdate: string, username: string): Promise<User> {
        ...
        if (userBirth > today) throw new DateError()
        const isUpdated: Boolean = await this.dao.updateUser(name, surname, address, birthdate, username)
        const updatedUser = await this.dao.getUserByUsername(username)
        return updatedUser
    }
}
