...
class ProductRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            ...
            body("category").custom(value => value === Category.APPLIANCE || value === Category.LAPTOP || value === Category.SMARTPHONE),
            body("quantity").isInt().custom(value => value > 0),
            body("details").isString(),
            body("sellingPrice").isFloat().custom(value => value > 0),
            body("arrivalDate").isString().matches(/\d{4}-\d{2}-\d{2}|^$/),
        ...
        this.router.patch(
            ...
            body("quantity").isInt().custom(value => value > 0),
            body("changeDate").isString().matches(/\d{4}-\d{2}-\d{2}|^$/),
            this.errorHandler.validateRequest,
        ...
        this.router.patch(
            ...
            this.authenticator.isAdminOrManager,
            param("model").isString().notEmpty(),
            body("sellingDate").isString().matches(/\d{4}-\d{2}-\d{2}|^$/),
            body("quantity").isInt().custom(value => value > 0),
            this.errorHandler.validateRequest,
        ...
        this.router.get(
            ...
            this.authenticator.isLoggedIn,
            this.authenticator.isAdminOrManager,
            query("category").custom(value => !value || value === Category.APPLIANCE || value === Category.LAPTOP || value === Category.SMARTPHONE),
            query("grouping").custom((value, {req} )=> {
                if (!value && !req.query.category && !req.query.model) {
                    return true
                } else if (value === "category" && req.query.category && !req.query.model) {
                    return true
                } else if (value === "model" && !req.query.category && req.query.model) {
                    return true
                } else {
                    return false
                }
